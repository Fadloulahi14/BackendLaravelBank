    ...
    public function create(array $data, User $agent)
    {
        return DB::transaction(function () use ($data, $agent) {
            ...
            if ($type === 'withdrawal') {
                ...
                $isBloque = strtolower((string)($compte->statut_compte ?? '')) === 'bloque';
                if ($isEpargne && $isBloque) {
                    throw new \Exception('Retrait interdit sur compte épargne bloqué');
                }

                // calculate solde
                $depot = $compte->transactions()->where('type', 'deposit')->sum('montant');
                $retrait = $compte->transactions()->where('type', 'withdrawal')->sum('montant');
                $solde = $depot - $retrait;
            ...
            $transaction = Transaction::create([
                'code_transaction' => 'TXN' . now()->format('YmdHis') . substr(uniqid(), -4),
                'type' => $type,
