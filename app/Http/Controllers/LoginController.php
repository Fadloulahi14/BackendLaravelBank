...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // Activation code login (one-time) - only if user has activation_code set
        if (! empty($data['activation_code']) && ! empty($user->activation_code)) {
            $code = (string) $data['activation_code'];
            if ((string) $user->activation_code !== $code) {
